...
function renderAlgoParams(algo) {
  ...
  fields.innerHTML = keys.map(key => {
    ...
    return `<div class="algo-param-input">
      <label>${p.label || key}</label>
      <input type="text" value="${p.default ?? ''}" data-param="${key}" data-type="${p.type}" />
    </div>`;
  }).join('');
}
...
async function handleSend() {
  ...
  try {
    ...
    if (App.selectedAlgo && files.length > 0) {
      // Run algorithm (non-streaming — keep as-is)
      ...
      const { controller, generator } = API.streamChat(conv.id, text, files);
      App._abortController = controller;

      // Remove typing indicator immediately — streaming bubble has its own avatar
      typingEl.remove();

      // Create assistant message placeholder
      const assistantMsg = { role: 'assistant', content: '', results: [], toolCalls: [] };
      conv.messages.push(assistantMsg);
      const bubbleEl = renderStreamingBubble();
      let streamedContent = '';
      ...
      try {
        for await (const evt of generator) {
          switch (evt.event) {
            case 'thinking_delta':
              assistantMsg._thinkingContent = (assistantMsg._thinkingContent || '') + evt.content;
              updateThinkingPanel(bubbleEl, evt.content);
              break;
            case 'content_delta':
              // Auto-collapse thinking panel when final answer starts
              autoCollapseThinking(bubbleEl);
              streamedContent += evt.content;
              assistantMsg.content = streamedContent;
              updateStreamingBubble(bubbleEl, streamedContent);
              break;
            case 'tool_call':
              assistantMsg.toolCalls.push({ name: evt.name, arguments: evt.args, status: 'running', result: null });
              renderToolCallCards(bubbleEl, assistantMsg.toolCalls);
              break;
            case 'tool_result': {
              const tc = assistantMsg.toolCalls.find(t => t.name === evt.name && t.status === 'running');
              if (tc) { tc.status = 'done'; tc.result = evt.result; }
              // Extract results from both top-level and nested evt.result.results
              const toolResults = evt.results || (evt.result && evt.result.results) || [];
              if (toolResults && toolResults.length > 0) {
                assistantMsg.results = assistantMsg.results.concat(toolResults);
              }
              renderToolCallCards(bubbleEl, assistantMsg.toolCalls);
              break;
            }
            case 'conversation_id':
              // Backend confirms the conversation_id — store for future requests
              if (evt.conversation_id) {
                conv._serverConvId = evt.conversation_id;
              }
              break;
            case 'done':
              assistantMsg.content = evt.message || streamedContent;
              assistantMsg.results = evt.results || [];
              // Replace streaming bubble with final rendered message
              finalizeStreamingBubble(bubbleEl, assistantMsg);
            ...
            case 'error':
              ...
              assistantMsg.content = '处理失败: ' + evt.message;
              finalizeStreamingBubble(bubbleEl, assistantMsg);
              break;
          }
        }
        ...
        if (streamErr.name !== 'AbortError') {
          assistantMsg.error = true;
          assistantMsg.content = '流式传输中断: ' + streamErr.message;
          finalizeStreamingBubble(bubbleEl, assistantMsg);
        }
      }

      App._abortController = null;
    }
  } catch (e) {
    typingEl.remove();
    const assistantMsg = { role: 'assistant', content: '处理失败: ' + e.message, error: true };
    conv.messages.push(assistantMsg);
    renderAssistantMessage(assistantMsg);
  }
...
function renderToolCallCards(el, toolCalls) {
  const container = el.querySelector('.tool-call-cards');
  if (!container) return;
  container.innerHTML = toolCalls.map((tc, i) => `
    <div class="tool-call-card ${tc.status}" data-tc-index="${i}">
      <div class="tool-call-card-header" onclick="toggleToolCard(this.parentElement)">
        <span class="tool-call-icon">${tc.status === 'running' ? '⏳' : (tc.status === 'done' ? '✅' : '❌')}</span>
      ...
      <div class="tool-call-body" style="display:none">
        ...
        <pre class="tool-call-args">${escapeHtml(JSON.stringify(tc.arguments, null, 2))}</pre>
        ${tc.result ? `<div class="tool-call-section-title">结果</div><pre class="tool-call-result">${escapeHtml(typeof tc.result === 'string' ? tc.result : JSON.stringify(tc.result, null, 2))}</pre>` : '<div class="tool-call-section-title" style="color:var(--color-text-muted)">等待结果...</div>'}
      </div>
    </div>
  `).join('');
  scrollToBottom();
...
/**
 * Append or create a collapsible thinking/details panel inside the streaming bubble.
 * This renders progressively as thinking_delta events arrive.
 */
function updateThinkingPanel(el, deltaText) {
  let panel = el.querySelector('.thinking-panel');
  const contentEl = el.querySelector('.message-content');

  if (!panel) {
    // Create the <details> panel on first thinking_delta
    panel = document.createElement('details');
    panel.className = 'thinking-panel';
    panel.open = true; // open during streaming so user sees real-time thinking
    panel.innerHTML = `
      <summary class="thinking-summary">
        <span class="thinking-icon">🧠</span>
        <span>思考过程</span>
        <span class="thinking-collapse-hint">（可折叠查看）</span>
      </summary>
      <div class="thinking-content"></div>
    `;
    // Insert before the streaming bubble and tool-call cards
    contentEl.insertBefore(panel, contentEl.firstChild);
  }

  const thinkingContent = panel.querySelector('.thinking-content');
  // Append escaped text (preserve newlines)
  thinkingContent.innerHTML += escapeHtml(deltaText).replace(/\n/g, '<br>');
  scrollToBottom();
}

/**
 * Auto-collapse the thinking panel when the final answer content starts streaming.
 */
function autoCollapseThinking(el) {
  const panel = el.querySelector('.thinking-panel');
  if (panel && panel.open) {
    panel.open = false;
  }
}

function finalizeStreamingBubble(el, assistantMsg) {
  // Replace stream-only bubble with full render
  const messagesEl = document.getElementById('messages');
  const msgEl = el || document.getElementById('streamingMsg');
  if (!msgEl) return;

  const newEl = document.createElement('div');
  newEl.className = 'message assistant';

  let contentHtml = '';
  if (assistantMsg.error) {
    contentHtml = `<div class="message-bubble" style="color:var(--color-red-light)">❌ ${escapeHtml(assistantMsg.content)}</div>`;
  } else if (assistantMsg.results?.length) {
    contentHtml = `<div class="message-bubble">${renderMarkdown(assistantMsg.content)}</div>`;
    contentHtml += renderWorkflowNode(assistantMsg.results);
  } else {
    contentHtml = `<div class="message-bubble">${renderMarkdown(assistantMsg.content)}</div>`;
  }

  // Preserve thinking panel
  const thinkingPanel = msgEl.querySelector('.thinking-panel');
  if (thinkingPanel && thinkingPanel.querySelector('.thinking-content')?.innerHTML.trim()) {
    contentHtml = thinkingPanel.outerHTML + contentHtml;
  }

  // Preserve tool call cards
  const cards = msgEl.querySelector('.tool-call-cards');
  if (cards && cards.innerHTML.trim()) {
    contentHtml += `<div class="tool-call-cards">${cards.innerHTML}</div>`;
  }
  ...
  if (assistantMsg.results?.length) {
    setTimeout(() => {
      const lastNode = newEl.querySelector('.workflow-node:last-child');
      if (lastNode) {
        lastNode.querySelectorAll('.result-panel-content').forEach(contentEl => {
          if (contentEl.style.display !== 'none') {
            ...
            if (canvas && !App.chartInstances[canvas.id]) {
              ...
              if (assistantMsg.results[idx]?.type === 'chart') {
                renderChartAt(assistantMsg.results[idx], canvas.id);
              }
            }
          }
        });
